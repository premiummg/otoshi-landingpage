...
// Google Places). Ported from Reel-FishR's own AddressAutocomplete
// (Reel-FishR-React-Native/src/lib/nominatim.ts) - same API, same rate-limit
// discipline, extended here with the structured city/province/postal-code
// fields that app didn't need but this registration form does (it autofills
// City/Province/Postal Code, not just one address line).
//
// Usage policy: https://operations.osmfoundation.org/policies/nominatim/
// asks for a way to identify the app - a custom User-Agent is the documented
// way, but browsers block a fetch call from actually setting that header
// and send the page's own Referer instead, which the policy also accepts.
// The header stays in the request below anyway (harmless, dropped
// silently) so this file matches its Reel-FishR original and needs no
// changes if it's ever reused somewhere the header isn't blocked.
...
// New Brunswick's own municipalities carry their legal designation right in
// Nominatim's `address.city` ("City of Moncton", "Town of Riverview") -
// correct, but not how anyone actually fills in a "City" field on a form.
...
// either had a chance to update it - chaining onto this promise instead of
// a plain check serializes every call through the same gate, one at a time.
...
export async function searchAddress(query: string): Promise<AddressSuggestion[]> {
  ...
  // countrycodes=ca - this club has one Dieppe, NB location; there's no
  // real case for this form autocompleting an address outside Canada.
  // addressdetails=1 so each result includes the structured fields below.
  const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&countrycodes=ca&limit=5&q=${encodeURIComponent(trimmed)}`;

  let response: Response;
  try {
    response = await fetch(url, {
      headers: {
        'User-Agent': 'OtoshiRegistration/1.0 (otoshi.ca)',
        'Accept-Language': 'en',
      },
    });
  } catch (error) {
